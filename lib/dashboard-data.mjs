...
/** Build ticker rows from category payloads — avoids ~9 duplicate Yahoo calls. */
function buildTickerFromCategories(categories) {
  const c = categories.crypto?.workspace?.metrics ?? [];
  const f = categories.forex?.workspace?.metrics ?? [];
  const s = categories.stocks?.workspace?.metrics ?? [];
  const o = categories.commodities?.workspace?.metrics ?? [];

  const items = [
    findMetric(c, 'Ethereum (ETH)') && {
      id: 'eth',
      name: 'Ethereum',
      value: findMetric(c, 'Ethereum (ETH)').value,
      changePct: findMetric(c, 'Ethereum (ETH)').changePct,
      category: 'crypto',
    },
    findMetric(c, 'Bitcoin (BTC)') && {
      id: 'btc',
      name: 'Bitcoin',
      value: findMetric(c, 'Bitcoin (BTC)').value,
      changePct: findMetric(c, 'Bitcoin (BTC)').changePct,
      category: 'crypto',
    },
    findMetric(o, 'WTI Oil') && {
      id: 'wti',
      name: 'WTI Oil',
      value: findMetric(o, 'WTI Oil').value,
      changePct: findMetric(o, 'WTI Oil').changePct,
      category: 'commodity',
    },
    findMetric(o, 'Gold') && {
      id: 'gold',
      name: 'Gold',
      value: findMetric(o, 'Gold').value,
      changePct: findMetric(o, 'Gold').changePct,
      category: 'commodity',
    },
    findMetric(f, 'EUR/USD') && {
      id: 'eurusd',
      name: 'EUR/USD',
      value: findMetric(f, 'EUR/USD').value,
      changePct: findMetric(f, 'EUR/USD').changePct,
      category: 'fx',
    },
    findMetric(s, 'NASDAQ 100') && {
      id: 'nasdaq',
      name: 'NASDAQ 100',
      value: findMetric(s, 'NASDAQ 100').value,
      changePct: findMetric(s, 'NASDAQ 100').changePct,
      category: 'index',
    },
    categories.stocks?.workspace?.primaryValue != null && {
      id: 'sp500',
      name: 'S&P 500',
      value: categories.stocks.workspace.primaryValue,
      changePct: categories.stocks.card?.changePct ?? 0,
      category: 'index',
    },
    findMetric(s, 'Dow Jones') && {
      id: 'dow',
      name: 'Dow Jones',
      value: findMetric(s, 'Dow Jones').value,
      changePct: findMetric(s, 'Dow Jones').changePct,
      category: 'index',
    },
    findMetric(s, 'Nikkei 225') && {
      id: 'nikkei',
      name: 'Nikkei 225',
      value: findMetric(s, 'Nikkei 225').value,
      changePct: findMetric(s, 'Nikkei 225').changePct,
      category: 'index',
    },
  ].filter(Boolean);
...
async function fetchExtraTickerSymbols() {
  const extras = [
    { id: 'dax', name: 'DAX', sym: '^GDAXI', category: 'index' },
    { id: 'ftse', name: 'FTSE 100', sym: '^FTSE', category: 'index' },
    { id: 'hsi', name: 'Hang Seng', sym: '^HSI', category: 'index' },
  ];

  const results = await Promise.all(
    extras.map(async (item) => {
      const q = await yahooFetch(item.sym);
...
export async function buildDashboard() {
  ...
  const [crypto, forex, stocks, commodities, insights, news, headlines] = await Promise.all([
    fetchCrypto(),
    fetchForex(),
    fetchStocks(),
    fetchCommodities(),
  ...
  const tickerItems = [...buildTickerFromCategories(categories), ...(await fetchExtraTickerSymbols())];
  const ticker = {
    row1: tickerItems.slice(0, 6),
    row2: tickerItems.slice(6),
  };
