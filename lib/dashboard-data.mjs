...
async function fetchCrypto() {
  ...
  if (!quotes.btc || !quotes.eth) {
    const [yBtc, yEth, ySol] = await Promise.all([
      yahooChart('BTC-USD'),
      yahooChart('ETH-USD'),
      yahooChart('SOL-USD'),
    ]);
    if (!yBtc || !yEth) return null;
    quotes = {
      btc: { value: yBtc.value, changePct: yBtc.changePct },
      eth: { value: yEth.value, changePct: yEth.changePct },
      sol: ySol ? { value: ySol.value, changePct: ySol.changePct } : null,
      xrp: quotes.xrp,
    };
    if (!chartData.length && yBtc.chartData?.length) chartData = yBtc.chartData;
  }

  if (!quotes.btc || !quotes.eth) return null;

  if (!quotes.xrp?.value) {
    const yXrp = await yahooChart('XRP-USD');
    if (yXrp) quotes.xrp = { value: yXrp.value, changePct: yXrp.changePct };
  }
...
export async function buildDashboard() {
  const [crypto, forex, stocks, commodities, ticker, insights, news, headlines] = await Promise.all([
    fetchCrypto(),
    fetchForex(),
    fetchStocks(),
    fetchCommodities(),
