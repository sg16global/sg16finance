/** Live market data — Binance (crypto), Yahoo (stocks/commodities), Frankfurter (forex). No API keys. */
...
async function fetchCrypto() {
  const [btc, eth, sol, xrp, btcKlines] = await Promise.all([
    binance24hr('BTCUSDT'),
    binance24hr('ETHUSDT'),
    binance24hr('SOLUSDT'),
    binance24hr('XRPUSDT'),
  ...
  const chartData = btcKlines?.slice(-16) ?? [];

  return {
    card: { statLabel: 'BTC 24h', changePct: btc.changePct, sparkline: chartData.slice(-14) },
    workspace: {
      primaryValue: btc.value,
      primaryLabel: 'Bitcoin (BTC)',
      secondaryValue: eth.value,
      secondaryLabel: 'Ethereum (ETH)',
      chartData,
      metrics: [
        { label: 'Bitcoin (BTC)', value: btc.value, changePct: btc.changePct },
        { label: 'Ethereum (ETH)', value: eth.value, changePct: eth.changePct },
        { label: 'Solana (SOL)', value: sol?.value ?? 0, changePct: sol?.changePct ?? 0 },
        { label: 'XRP (XRP)', value: xrp?.value ?? 0, changePct: xrp?.changePct ?? 0 },
      ],
    },
  };
}
...
async function fetchTicker() {
  const items = [
    { id: 'eth', name: 'Ethereum', fetch: () => binance24hr('ETHUSDT'), category: 'crypto' },
    { id: 'btc', name: 'Bitcoin', fetch: () => binance24hr('BTCUSDT'), category: 'crypto' },
